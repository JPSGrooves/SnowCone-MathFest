...
// 5) SPLIT — “split $60 three ways”, “split 90 by 4”
function solveSplit(t) {
  const m = /split\s*\$?\s*(\d+(?:\.\d+)?)\s*(?:by|between|among|into)?\s*(\d+)\s*(?:people|ways|parts)?/i.exec(t);
  if (!m) return null;
  const total = toNum(m[1]), n = Math.max(1, parseInt(m[2], 10));
  const each = total / n;
...
function solvePercentChange(t) {
  ...
  const step = `percent change from ${fmtNum(oldV)} to ${fmtNum(newV)} → (${fmtNum(newV)} − ${fmtNum(oldV)}) ÷ ${fmtNum(oldV)} × 100% = <strong>${fmtNum(delta)}%</strong>`;
  return { html: `<p>${step}</p>` };
}
...
export function handle(text = '') {
  ...
  for (const f of solvers) {
    const hit = f(raw);
    if (hit) {
      const reply = hit.html;
      if (S.isCapReached()) {
        return {
          html: composeReply({
            part: { html: `${reply}<p>calculator cool-down. try another booth?</p>` },
            askAllowed: false,
            noAck: true
          })
        };
  ...
  try {
    ...
    const line = `<p>${raw} = <strong>${fmtNum(val)}</strong></p>`;
    if (S.isCapReached()) {
    ...
    const hint = `<p>couldn’t parse. try <code>27% off $10</code>, <code>add 18% tip to $42</code>, <code>split $60 three ways</code>, or <code>3/8 as decimal</code>.</p>`;
